                  ...
                  proposals.map((p) => (
                    <div key={p.id} className="glass-card" style={{ padding: "1rem", background: "rgba(0,0,0,0.2)", border: "1px solid rgba(255,255,255,0.05)", marginBottom: "0.5rem" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <div>
                          <strong>{p.freelancer_name}</strong>
                          <div style={{ fontSize: "0.85rem", color: "var(--color-text-muted)", marginTop: "0.25rem" }}>
                            Proposed ${Number(p.rate).toLocaleString()} • {new Date(p.created_at).toLocaleDateString()}
                          </div>
                        </div>
                      ...
                      {selectedProposal?.id === p.id && (
                        <div style={{ marginTop: "1rem", paddingTop: "1rem", borderTop: "1px solid rgba(255,255,255,0.1)" }}>
                          ...
                          <div style={{ marginTop: "1rem", display: "flex", gap: "0.5rem" }}>
                            <Link href={`/profile/${p.freelancer_id}`} className="btn btn--secondary btn--sm">View Profile</Link>
                            <button className="btn btn--primary btn--sm">Accept Proposal</button>
                          </div>
                        </div>
                      )}
                    </div>
